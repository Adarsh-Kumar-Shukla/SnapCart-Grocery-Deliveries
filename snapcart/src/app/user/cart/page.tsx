    ...
    <div className='w-[95%] sm:w-[90%] md:w-[80%] mx-auto mt-8 mb-24 relative'>
      ...
      {cartData.length == 0 ? (
        ...
        >
          ...
          <p className='text-gray-400 text-lg mb-6'>Your Cart is Empty. Add some groceries to continue shopping!</p>
          <Link href={"/"} className='bg-green-600 text-white px-6 py-3 mt-3 rounded-full hover:bg-green-700 transition-all inline-block font-medium'>Continue Shopping</Link>
        </motion.div>
      ) : (
          ...
          <div className='lg:col-span-2 space-y-5'>
            <AnimatePresence>
              {cartData.map((item,index)=>(
                ...
                transition={{duration:0.5}}
                exit={{opacity:0,y:-20}}
                className='flex flex-col sm:flex-row items-center bg-white rounded-2xl shadow-md p-5 hover:shadow-xl transition-all duration-300 border border-gray-100 mb-2'
                >
                  ...
                  <div className='mt-4 sm:mt-0 sm:ml-4 flex-1 text-center sm:text-left'>
                    ...
                    <p className='text-xs sm:text-sm text-gray-500'>{item.noOfItem} x {item.quantity} {item.unit}</p>
                    <p className='text-green-700 font-bold mt-1 text-sm sm:text-base'>₹{Number(item.price)*item.noOfItem}</p>
                  </div>
                  <div className='flex items-center justify-center sm:justify-end gap-3 mt-3 sm:mt-0 bg-gray-50 px-3 py-2 rounded-full'>
                    ...
                    <span className='font-semibold text-gray-800 w-6 text-center'>{item.noOfItem}</span>
