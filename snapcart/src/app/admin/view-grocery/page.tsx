...
import { IGrocery } from "@/models/grocery.models";
...
const ViewGrocery = () => {
  ...
  const [filltered, setFillttered] = useState<IGrocery[]>()
  const categories = [
  "Fruits & Vegetables",
  "Dairy & Eggs",
  "Rice, Atta & Grains",
...
const units = ["kg", "g", "liter", "ml", "piece", "pack"];
  ...
  const handleEdit = async ()=>{
    ...
    try {
      ...
      formData.append("quantity", editing.quantity);
  ...
  const handleDelete = async ()=>{
    ...
    try {
      ...
      const result = await axios.post("/api/admin/delete-grocery", {groceryId});
        ...
        >
          ...
          >
            ...
            <div className="relative aspect-square w-full rounded-lg overflow-hidden mb-4 border border-gray-200 group">
              ...
              className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center cursor-ponter transition-opacity"><Upload size={28} className="text-green-500"/></label>
              <input 
              type="file" 
              accept="image/*" 
            ...
            <div className="flex justify-end gap-3  mt-6">
              ...
              className="px-4 py-2 rounded-lg bg-green-600 text-white flex items-center gap-2 hover:bg-green-700 transition">{editLoading ? <Loader size={14} className="animate-spin"/> : "Edit Grocdry"}</button>
              <button 
              onClick={handleDelete}
              disabled={deleteLoading}
              className="px-4 py-2 rounded-lg bg-red-600 text-white flex items-center gap-2 hover:bg-red-700 transition">{deleteLoading ? <Loader size={14} className="animate-spin"/> : "Delete Grocery"}</button>
            </div>
          </motion.div>
        </motion.div>}
      </AnimatePresence>
