...
const AdminDashBoardClient = ({earning, stats, chartData}:propsType) => {

  const [filter,setFilter]=useState<"today" | "sevenDays" | "total">()

  const currenEarning=filter==="today" ? earning.today:filter==="sevenDays" ? earning.sevenDays:earning.total
  ...
  return (
    <div className='pt-28 w-[90%] md:w-[80%] mx-auto '>
      ...
      >
        ...
        <h2 className='text-4xl font-extrabold text-green-800'>₹{currenEarning.toLocaleString()}</h2>
      </motion.div>
