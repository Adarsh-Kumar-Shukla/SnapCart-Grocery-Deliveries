...
const Nav = ({ user }: { user: IUser }) => {
  ...
  const sideBar = menuOpen ? createPortal(
    <AnimatePresence>
      ...
      className="fixed top-0 left-0 h-full w-[75%] sm:w-[60%] z-9999 bg-linear-to-b from-green-800/90 via-green-700/80 to-green-900/90 backdrop-blur-xl border-r border-green-400/20 shadow-[0_0_50px_-10px_rbga(0,255,100,0.3)] flex flex-col p-6 text-white"
      >
        ...
        <div className="flex items-center gap-3 mt-3 rounded-xl bg-white/10 hover:bg-white/15 transition-all shadow-inner">
          <div className="relative w-12 h-12 rounded-full overflow-hidden border-2 border-green-400/60 shadow-lg">
            ...
            <div>
              <h2 className="text-lg font-semibold capitalize text-white">{user.name}</h2>
              <p className="text-xs text-green-200 capitalize tracking-wide">{user.role}</p>
            </div>
        </div>
        <div className="flex flex-col gap-3 font-medium mt-6">
          <Link href={"/admin/add-grocery"} className="flex items-center pag-3 p-3 rounded-full bg-white/10 hover:bg-white/20 hover:pl-4 transition-all"><PlusCircle className="w-5 h-5"/> Add Grocery</Link>
            <Link href={"/admin/view-grocery"} className="flex items-center pag-3 p-3 rounded-full bg-white/10 hover:bg-white/20 hover:pl-4 transition-all"><Boxes className="w-5 h-5"/> View Grocery</Link>
            <Link href={"/admin/manage-orders"} className="flex items-center pag-3 p-3 rounded-full bg-white/10 hover:bg-white/20 hover:pl-4 transition-all"><ClipboardCheck className="w-5 h-5"/> Manage Orders</Link>
        </div>
        <div className="my-5 border-t border-white/20"></div>
        <div className="flex items-center gap-3 text-red-300 font-semibold mt-auto hover:bg-red-500/20 p-3 rounded-full transition-all" onClick={async()=>await signOut({callbackUrl:"/"})}>
  ...
  return (
    <div className="w-[90%] fixed top-4 left-1/2 -translate-x-1/2 bg-linear-to-r from-green-500 to-green-700 rounded-2xl shadow-lg shadow-black/30 flex justify-between items-center h-20 px-4 md:px-8 z-50">
      <Link
        href={"/"}
      ...
      <div className="flex items-center gap-3 md:gap-6 relative">
        ...
        {user.role == "admin" && <>
          ...
          <div className="md:hidden bg-white rounded-full w-10 h-10 flex items-center justify-center shadow-md" onClick={()=>setMenuOpen(prev=>!prev)}>
            <Menu className="text-green-600 w-6 h-6 flex items-center justify-center shadow-md"/>
          </div>
        </>}

        <div className="relative" ref={profileDropDown}>
          <div
            className="bg-white relative rounded-full w-11 h-11 flex items-center justify-center overflow-hidden shadow-md hover:scale-105 transition-transform relative"
          ...
          <AnimatePresence>
            {searchBarOpen && (
              ...
              >
                <SearchIcon className="text-gray-500 h-5 mr-2" />
